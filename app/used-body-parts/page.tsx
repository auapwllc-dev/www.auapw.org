...
import { Search, Phone, MessageSquare, Shield, Truck, Clock, Car } from "lucide-react"

const CATEGORY = PART_CATEGORIES.find(c => c.id === "body")!

export default function UsedBodyPartsPage() {
  return (
    <>
      <Navbar />
      <main className="pt-[58px]">
        ...
        <div className="bg-gradient-to-br from-background via-card to-background border-b border-border/30">
          ...
          <div className="mx-auto max-w-[1280px] px-6 py-14">
            <div className="flex items-center gap-2 text-xs text-muted-foreground mb-6">
              ...
              <span>/</span>
              <span className="text-foreground">Body & Interior</span>
            </div>
            <div className="flex items-center gap-4 mb-5">
              <div className="w-8 h-px bg-gradient-to-r from-transparent to-primary/50" />
              <span className="text-[0.65rem] font-bold tracking-[0.3em] uppercase text-primary">Body Parts</span>
            </div>
            <h1 className="font-serif text-[clamp(1.75rem,4vw,3.5rem)] font-bold text-foreground">Used Body & Interior Parts</h1>
            <p className="mt-3 text-sm text-muted-foreground max-w-[520px]">
              Doors, hoods, bumpers, fenders, headlights, seats, and all body components from 2,000+ verified salvage yards.
            </p>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-[1280px] px-6 py-8">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { icon: Shield, title: "Up to 6-Month Warranty", desc: "Every body part covered with full return & replacement" },
        ...
        {/* All Body Parts */}
        <div className="mx-auto max-w-[1280px] px-6 py-8">
          <h2 className="text-2xl font-bold mb-6">All Body & Interior Parts</h2>
        ...
        {/* Shop by Make */}
        <div className="mx-auto max-w-[1280px] px-6 py-8">
          <h2 className="text-2xl font-bold mb-6">Shop Body Parts by Make</h2>
          <div className="flex flex-wrap gap-2">
            {CAR_MAKES.map((make) => (
              <Link
                key={make}
                href={`/search?make=${encodeURIComponent(make)}&category=body`}
        ...
        <div className="mx-auto max-w-[1280px] px-6 py-12">
          <div className="text-center embossed-col rounded-lg p-8">
            <h3 className="font-serif text-xl font-semibold text-foreground mb-2">Need Help Finding a Body Part?</h3>
