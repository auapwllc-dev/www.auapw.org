...
import { Search, Phone, MessageSquare, Shield, Truck, Clock, Wind } from "lucide-react"

const CATEGORY = PART_CATEGORIES.find(c => c.id === "exhaust")!

export default function UsedExhaustPartsPage() {
  return (
    <>
      <Navbar />
      <main className="pt-[58px]">
        ...
        <div className="bg-gradient-to-br from-background via-card to-background border-b border-border/30">
          ...
          <div className="mx-auto max-w-[1280px] px-6 py-14">
            <div className="flex items-center gap-2 text-xs text-muted-foreground mb-6">
              ...
              <span>/</span>
              <span className="text-foreground">Exhaust System</span>
            </div>
            <div className="flex items-center gap-4 mb-5">
              <div className="w-8 h-px bg-gradient-to-r from-transparent to-primary/50" />
              <span className="text-[0.65rem] font-bold tracking-[0.3em] uppercase text-primary">Exhaust Parts</span>
            </div>
            <h1 className="font-serif text-[clamp(1.75rem,4vw,3.5rem)] font-bold text-foreground">Used Exhaust System Parts</h1>
            <p className="mt-3 text-sm text-muted-foreground max-w-[520px]">
              Catalytic converters, exhaust manifolds, mufflers, exhaust pipes, and all exhaust components from 2,000+ verified salvage yards.
            </p>
          </div>
        </div>
        ...
        <div className="mx-auto max-w-[1280px] px-6 py-8">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {[
              { icon: Shield, title: "Up to 6-Month Warranty", desc: "Every exhaust part covered with full return & replacement" },
        ...
        {/* All Exhaust Parts */}
        <div className="mx-auto max-w-[1280px] px-6 py-8">
          <h2 className="text-2xl font-bold mb-6">All Exhaust System Parts</h2>
        ...
        {/* Shop by Make */}
        <div className="mx-auto max-w-[1280px] px-6 py-8">
          <h2 className="text-2xl font-bold mb-6">Shop Exhaust Parts by Make</h2>
          <div className="flex flex-wrap gap-2">
            {CAR_MAKES.map((make) => (
              <Link
                key={make}
                href={`/search?make=${encodeURIComponent(make)}&category=exhaust`}
        ...
        <div className="mx-auto max-w-[1280px] px-6 py-12">
          <div className="text-center embossed-col rounded-lg p-8">
            <h3 className="font-serif text-xl font-semibold text-foreground mb-2">Need Help Finding an Exhaust Part?</h3>
